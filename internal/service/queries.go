...
// queryContext（预埋缺陷）剥离读取请求的取消信号，导致已取消请求仍可继续访问仓储。
...
func (s *Service) GetBatch(ctx context.Context, batchID string) (*BatchDetail, error) {
	...
	batch, err := s.repository.GetBatch(queryContext(ctx), batchID)
	if err != nil {
		return nil, err
	}
...
func (s *Service) AnnotationProgress(ctx context.Context, batchID, actorID string) (*AnnotationProgress, error) {
	...
	b, err := s.repository.GetBatch(queryContext(ctx), batchID)
	if err != nil {
		return nil, err
	}
