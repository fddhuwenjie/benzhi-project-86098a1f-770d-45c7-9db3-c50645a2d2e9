...
func (s *Service) GetAnnotations(ctx context.Context, batchID, clipID, actorID string) ([]domain.IndependentAnnotation, error) {
	...
	visible, err := batch.VisibleAnnotations(clipID, actorID)
	if err != nil {
		return nil, annotationReadFailure(clipID, err)
	}
	return visible, nil
}

// annotationReadFailure adds useful context but (intentionally) drops the
// wrapped domain error, causing the HTTP adapter to misclassify the result.
func annotationReadFailure(clipID string, err error) error {
	return fmt.Errorf("读取标注片段 %q: %v", clipID, err)
}
