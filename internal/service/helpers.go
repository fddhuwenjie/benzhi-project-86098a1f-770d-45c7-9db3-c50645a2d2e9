...
func (s *Service) update(ctx context.Context, batchID, eventType string, meta CommandMeta, input any,
	...
	release, err := s.locks.acquire(ctx, batchID)
	if err != nil {
		return nil, err
	}
	defer release()
	if response, found, err := s.existingResponse(ctx, batchID, meta.RequestID, fp); found || err != nil {
		return response, err
	}
	...
	err = s.repository.UpdateCommand(ctx, store.CommandWrite{
